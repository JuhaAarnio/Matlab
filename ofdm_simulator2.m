function [BER_qam, BER_enc, SNR] = ofdm_simulator2(bitlenght, iterations)
...
for j = 1:iterations
    ...
    for i = 1:(cw_lenght/8)
        ...
        %Saving data for channel estimation.
        a = QAM_vector;
        %Performing IFFT before sending the data over the channel.
        QAM_ifft = ifft(QAM_vector);
        [h, sent] = channelEx6(QAM_ifft);
        %Saving data for channel estimation.
        b = fft(sent);
        sent_awgn = awgn(sent, dB, 'measured');
        sent_awgn_fft = fft(sent_awgn);
        %calculating the channel estimate.
        channel_estimate = a./b;
        %Applying the channel estimation.
        sent_estimated = sent_awgn_fft .* channel_estimate;
        decoded = [];
        QAM_vector(1:4) = [];
        for k = 1:4
            ...
            %calculations.
            received1 = qamdemod(sent_estimated(1+4:255+4), 256);
            received2 = qamdemod(sent_estimated(256+4:510+4), 256);
            received3 = qamdemod(sent_estimated(511+4:765+4), 256);
            received4 = qamdemod(sent_estimated(766+4:1020+4), 256);
